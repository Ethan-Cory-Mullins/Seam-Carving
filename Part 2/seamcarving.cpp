...
int main() {
  ...
  int targetWidth = 0;
  int targetHeight = 0;
  unsigned int twid, the; // going to assign the unsigned int to them so I can compoaore them 
  ...
  if (targetHeight > 1080 || targetWidth > 1920 ) { // if target is higher than the max limits, then stop the program 
    cout << "Invalid target dimensions";
    return 1;
  }
  twid = targetWidth;
  the = targetHeight;
  if (the > height || twid > width) { // if targetheigth targetwith are higher thann the hieght and width 
    cout << "Invalid target dimensions";
    return 1;
  }
